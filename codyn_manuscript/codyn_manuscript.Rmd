...
#####Turnover
Average annual species richness was higher in the unburned site at Konza, although richness varied a fair amount over time in both sites (Figure 1a). Total turnover indicates that, in addition to higher overall richness, there were also greater fluctuations in the species present in the unburned than burned location at Konza (Figure 1b). This insight helps to contextualize the richness patterns; even between consequentive years with similar species richness, the identity of the species composite species could differ by 30%. Higher species richness in the unburned plots can also be related to the higher overall proportional species appearances in the unburned plots (Figure 2b).
...
### Worked example: Konza prairie long-term dynamics data set.

To demonstrate the utility of the `codyn` tools, we show how to calculate spatial and temporal diversity indices using data from [@Collins08]. These data represent a long-term monitoring of grassland plant diversity in annually burned and unburned plots.

First, looking at stability. The `community_stability` function calculates $1/CV$, across time points for each replicate. The annually burned plot exhibits higher stability of across time, while the unburned plot is undergoing succession. 
```{r}
library(codyn)
data("knz_001d")
...
Variance ratio
```{r results='asis'}
KNZ_variance_ratio <- variance_ratio(knz_001d, 
                                   abundance.var = "abundance", 
                                   bootnumber = 10, 
                                   replicate.var = "subplot")
kable(head(KNZ_variance_ratio))
```
...
Lauren to add: turnover, rank clocks, mean rank shifts, rate_change
=======
###Stability metrics at Konza
